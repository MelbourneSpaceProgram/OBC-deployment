...
our %config = (
  ...
  openssldir => "ssl",
  options => "enable-shared --prefix=/workspaces/OBC-deployment/openssl-1.1.1l/_output --openssldir=ssl --cross-compile-prefix=arm-linux-gnueabihf- no-afalgeng no-asan no-asm no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-makedepend no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.34.0",
  perlargv => [ "linux-elf", "no-asm", "shared", "--prefix=/workspaces/OBC-deployment/openssl-1.1.1l/_output", "--openssldir=ssl", "--cross-compile-prefix=arm-linux-gnueabihf-" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
  ...
  prefix => "/workspaces/OBC-deployment/openssl-1.1.1l/_output",
  processor => "",
